...
public class ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<ExceptionResponseBean> handleInvalidDataAccessApiUsageException (InvalidDataAccessApiUsageException ex, WebRequest request) {
		ExceptionResponseBean exceptionDetails = new ExceptionResponseBean(HttpStatus.NOT_ACCEPTABLE,new Date(), ex.getMessage(),
	      request.getDescription(false));
	  return new ResponseEntity<>(exceptionDetails, HttpStatus.NOT_ACCEPTABLE);
	}
	
	@ExceptionHandler(NumberFormatException.class)
	  public final ResponseEntity<ExceptionResponseBean> handleNumberFormatException(NumberFormatException ex, WebRequest request) {
		ExceptionResponseBean exceptionDetails = new ExceptionResponseBean(HttpStatus.NOT_ACCEPTABLE,new Date(), ex.getMessage(),
		 request.getDescription(false));
	    return new ResponseEntity<ExceptionResponseBean>(exceptionDetails,HttpStatus.NOT_ACCEPTABLE);
	  }
	
	@ExceptionHandler(ConnectException.class)
	  public final ResponseEntity<ExceptionResponseBean> handleConnectException(ConnectException ex, WebRequest request) {
		ExceptionResponseBean exceptionDetails = new ExceptionResponseBean(HttpStatus.GATEWAY_TIMEOUT,new Date(), ex.getMessage(),
		 request.getDescription(false));
	    return new ResponseEntity<ExceptionResponseBean>(exceptionDetails,HttpStatus.GATEWAY_TIMEOUT);
	  }
